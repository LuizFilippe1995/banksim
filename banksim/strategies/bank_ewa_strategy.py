class BankEWAStrategy:
    ...
    def get_gamma_value(self):
        return (self.gammaIndex + 3) / 100
        
    def __eq__(self, other):
        if isinstance(other, self.__class__):
            return self.alphaIndex == other.alphaIndex and self.betaIndex == other.betaIndex and self.gammaIndex == other.gammaIndex
        return False
    ...
    def bank_ewa_strategy_list(cls):
        return [BankEWAStrategy(a, b, c) for a in range(cls.numberAlphaOptions) for b in range(cls.numberBetaOptions) for c in range(cls.numberGammaOptions)]
